export const query = {
  ...
  getLeadByIdQuery: (id) =>
    `Select Lead_Id__c,Id,LeadSource,Status,RM_SM_Name__c,Channel_Name__c,Branch_Name__c,Employee_Code__c,Customer_Name__c,DSA_Code__c,Bank_Branch__c,Branch_Manager__c,Customer_Profile__c,MobilePhone,Alternative_Mobile_Number__c,FirstName,MiddleName,LastName,Email,Residential_Address__c,Pincode__c,Product__c,ProductLookup__c,Property_Identified__c,Requested_loan_amount__c,Requested_tenure_in_Months__c,CreatedDate,OwnerId,Is_OTP_Limit_Reached__c,Last_OTP_Attempt_Time__c,OTP_Attempts__c,OTP_Verified__c,Cust_ID__c,Physical_Application_Number__c from Lead WHERE Id='${id}' `,
...
,BanchBrch__c,BanchBrch__r.Name,BanchBrch__r.BrchCode__c,Id,IsActive__c,Name,RMUsr__c,RMUsr__r.Name,DSA_UGA_Code__c,DSAUGA__c FROM DSABrchJn__c WHERE  IsActive__c = true`,
  // (RMUsr__c = '${userId}' OR DSAUGA__c = '${userId}') AND
